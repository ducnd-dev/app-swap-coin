// filepath: app/scripts/coin-market-test.ts
...
async function testPriceData() {
  ...
  for (const token of TEST_TOKENS) {
    try {
      const result = await coinMarketAPI.getTokenPrice(token) as PriceData;
      
      if ('error' in result && result.error && 'message' in result) {
        logError(`Failed to get price for ${token}: ${result.message}`);
      } else {
        // Now TypeScript knows this is PriceData
...
async function testBlockchainData() {
  ...
  try {
    ...
    function isApiError(obj: unknown): obj is ApiErrorResponse {
      // Type guard function to safely check for error response    function isApiError(obj: unknown): obj is ApiErrorResponse {
